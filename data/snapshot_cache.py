...
class SnapshotCache:
    ...
    # ── İndikatör Hesaplama (IndicatorEngine'e Delege Edilir) ────────────────
    ...
    def compute_momentum(self, symbol: str, period: int = 10, tf: str = "1m") -> float:
        ...
        if not bars: return 0.0
        return IndicatorEngine.momentum([b.close for b in bars], period)

    def compute_boll(self, symbol: str, period: int = 20, std_dev: int = 2, tf: str = "1m"):
        bars = self.get_bars(symbol, tf, n=period)
        if not bars: return 0.0, 0.0, 0.0
        return IndicatorEngine.boll([b.close for b in bars], period, std_dev)
