...
class IndicatorEngine:
    ...
    def momentum(values: List[float], period: int = 10) -> float:
        ...
        if len(values) < period + 1:
            ...
            # Fallback to total change if not enough data
            return float(round((values[-1] - values[0]) / values[0] * 100, 3))
            
        start_val = float(values[-(period + 1)])
        if start_val == 0: return 0.0
        
        moma = (float(values[-1]) - start_val) / start_val * 100
        return float(round(moma, 3))
